...
(function () {
  ...
  /**
   * เคยลองดึงของชิ้นนี้ในรอบเบราว์เซอร์นี้แล้วหรือยัง
   *
   * ทุกหน้าในระบบดึงข้อมูลให้เองตอนเปิด เพื่อไม่ให้ใครต้องกดปุ่มก่อนถึงจะเห็นอะไร
   * แต่ต้องไม่กลายเป็นการถล่ม Apps Script ตอนที่มันมีปัญหาอยู่พอดี — เปิดสิบหน้า
   * แล้วยิงสิบครั้งซ้ำ ๆ กับสิ่งที่เพิ่งล้มไป
   *
   * แยกโควตาตามชื่อ เพราะออเดอร์ล้มไม่ควรปิดโอกาสของสต๊อก และหน้าหนึ่งล้มไม่ควร
   * ปิดโอกาสของอีกหน้า ส่วนกรณีสำเร็จไม่ต้องพึ่งตรงนี้อยู่แล้ว เพราะข้อมูลจะอยู่
   * ในแคชและหน้าถัดไปเห็นเอง
   *
   * @returns {boolean} true = ยังไม่เคยลอง (และจองสิทธิ์ลองไว้ให้แล้ว)
   */
  function firstTry(key) {
    var k = 'wms:autotried:' + key;
    try {
      if (sessionStorage.getItem(k)) return false;
      sessionStorage.setItem(k, '1');
      return true;
    } catch (e) {
      return true;                                 // โหมดส่วนตัว — ยอมให้ลอง ดีกว่าไม่ทำอะไรเลย
    }
  }
